...
def dispatch_once_free(
    ...
    *,
    ...
    failure_limit: int = DEFAULT_SPAWN_FAILURE_LIMIT,
    board: Optional[str] = None,
    plan_free_adapter=None,
) -> DispatchResult:
    """``dispatch_once`` variant that unfolds a candidate task-tree (depth >= 2).

    This function is the integration point for the Plan/Free port factorization
    (Gap G-S5-H3-1).  It delegates:

      * Tree unfold (candidate generation) → ``FreePort`` (BFSFreeAdapter)
      * Task ranking (policy / value iteration) → ``PlanPort`` (RICEPlanAdapter)

    The two ports are wired together by ``PlanFreeDispatchAdapter`` and share
    NO state.  The integration uses only the port contracts — no direct calls
    to ``_build_task_subtree`` or inline scoring logic remain here.

    Parameters
    ----------
    plan_free_adapter:
        Optional ``PlanFreeDispatchAdapter`` instance.  If ``None``, the
        default adapter (BFSFreeAdapter + RICEPlanAdapter) is used.
    """
    if os.name != "nt":
        try:
            while True:
    ...
    # ── Plan/Free port dispatch ──────────────────────────────────────────────
    # Stage 1 (FreePort) + Stage 2 (PlanPort) are delegated to the adapter.
    # No scoring logic lives here; dispatch_once_free uses ONLY port contracts.
    from hermes_kanban.ports.plan_free_ports import make_default_plan_free_adapter

    adapter = plan_free_adapter if plan_free_adapter is not None else make_default_plan_free_adapter()
    ranked_candidates = adapter.rank_ready_tasks(conn, ready_rows)

    # Populate tree_depth from the max descendant depth seen by FreePort
    max_reachable_depth = max(
        (c.subtree.max_descendant_depth for c in ranked_candidates),
        default=0,
    )
    result.tree_depth = max_reachable_depth

    # Convert ranked CandidateTask list back to row-indexed lookup for spawn loop
    # (spawn loop only needs task_id, assignee — both on CandidateTask)
    ranked = ranked_candidates

    spawned = 0
    for cand in ranked:
        # cand is a CandidateTask (from PlanPort) — use .task_id / .assignee
        if max_spawn is not None and spawned >= max_spawn:
            break
        if not cand.assignee:
            result.skipped_unassigned.append(cand.task_id)
            continue
        try:
            from hermes_kanban._adapters import profile_exists  # local import: avoids cycle
        except Exception:
            profile_exists = None  # type: ignore[assignment]
        if profile_exists is not None and not profile_exists(cand.assignee):
            result.skipped_nonspawnable.append(cand.task_id)
            continue
        if dry_run:
            result.spawned.append((cand.task_id, cand.assignee, ""))
            continue
        claimed = claim_task(conn, cand.task_id, ttl_seconds=ttl_seconds)
        if claimed is None:
            continue
        try:
            workspace = resolve_workspace(claimed, board=board)
