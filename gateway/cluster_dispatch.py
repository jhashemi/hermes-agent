...
def compute_out_of_scope_boards(
    ...
    * ``cluster_dispatch=True``: reports boards with active tickets NOT on
    ...
    # Enumerate boards from disk. Import lazily so cluster_dispatch stays
    # importable without a full CLI environment.
    try:
    ...
    for meta in boards:
        ...
        if cluster_enabled:
            # A non-empty whitelist means "route these boards to the cluster";
            # any active-work board NOT on the list is warn-worthy drift.
            # An EMPTY whitelist under cluster_dispatch=true means everything
            # falls back to local — that's a broader config bug, but not
            # per-board scope drift; skip individual board warns.
            ...
            # cluster_dispatch=false: local dispatcher still touches every
            # board, so this is informational, not a functional break.
            if whitelist and not in_whitelist:
                out.append({
                    "slug": slug,
                    "active_count": active,
    ...
    # Sort: warns first, then info; within each severity, alphabetical.
