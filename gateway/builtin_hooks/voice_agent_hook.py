...
class VoiceAgentMessageInterceptor(GatewayMessageHook):
    ...
    async def before_message_processing(
        ...
        # Voice hook intercepts ONLY voice-specific commands:
        #  1. /voice-agents, /voice-list, /voice-disconnect (always)
        #  2. /load-* ONLY if user already has an active voice session (reconnect)
        #  3. Audio/voice messages when user has active voice session
        # For /load-* without active session → falls through to agent_commands.py
        
        if text:
            if text.startswith("/load-") and self._sessions.has_session(user_id, platform):
                return await self._handle_voice_load(text, event, platform, user_id)
            elif text in ("/voice-agents", "/voice-list"):
    ...
    async def _handle_voice_load(
        ...
        """Handle /load-{agent} by proxying to voice bridge /load endpoint."""
