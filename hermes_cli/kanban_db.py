...
def block_task(
    ...
    * ``transient`` — treated like a generic block for routing, but a worker
    ...
    ``waiting_for`` / ``waiting_for_commit`` / ``waiting_for_event`` /
    ``waiting_for_condition`` (all optional, all schema-additive per
    VFE-NERVE-01) replace prose-only block reasons with structured
    handoff facts the kernel can reason about:

    * ``waiting_for`` — a ticket id this block is gated on. Its
      existence is validated up front; a phantom id raises
      :class:`MissingWaitingForError` and the task state is NOT mutated
      (an ``block_rejected_missing_waiting_for`` event is emitted so the
      rejection is auditable).
    * ``waiting_for_commit`` — a commit hash pending merge/CI.
    * ``waiting_for_event`` — a NATS/event-bus subject the block
      expects to fire (e.g. ``spine.gate.wave2_cleared``).
    * ``waiting_for_condition`` — a human-readable predicate a
      subsequent L3 rechecker can evaluate.

    All four are persisted on the emitted event payload (``blocked``,
    ``dependency_wait``, or ``block_loop_detected``) so downstream
    tooling (VFE-NERVE-02..05) can query them without re-parsing prose.
    ...
    # Build the typed handoff dict once — merged into every emitted
    # event payload below so downstream consumers only look in one
    # place regardless of block routing.
    def _handoff_payload() -> dict:
        out: dict = {}
        if waiting_for:
            out["waiting_for"] = waiting_for
        wf_commit = (
            str(waiting_for_commit).strip() if waiting_for_commit else None
        )
        if wf_commit:
            out["waiting_for_commit"] = wf_commit
        wf_event = (
            str(waiting_for_event).strip() if waiting_for_event else None
        )
        if wf_event:
            out["waiting_for_event"] = wf_event
        wf_condition = (
            str(waiting_for_condition).strip() if waiting_for_condition else None
        )
        if wf_condition:
            out["waiting_for_condition"] = wf_condition
        return out
    ...
    with write_txn(conn):
        ...
        if kind == "dependency":
            ...
            _append_event(
                ...
                {"reason": reason, "kind": kind, **handoff}, run_id=run_id,
            )
        ...
        if recurrences >= BLOCK_RECURRENCE_LIMIT:
            ...
            _append_event(
                ...
                {"reason": reason, "kind": kind, "recurrences": recurrences, **handoff},
                run_id=run_id,
            )
        _blocked_task = get_task(conn, task_id)
    _fire_kanban_lifecycle_hook(
...
# ---------------------------------------------------------------------------
# VFE-NERVE-02: cascade auto-unblock helper (called from complete_task).
# ---------------------------------------------------------------------------

# Block kinds that require an explicit human decision — cascade must never
# clear these silently.  ``dependency`` and ``transient`` (and legacy-None)
# cascade normally.  Kept as a module-level set (not a literal in the loop)
# so tests can override for edge-case coverage.
...
def _default_spawn(
    ...
    *,
    ...
    # NOTE: We intentionally do NOT set HERMES_HOME to the profile directory.
    # Setting HERMES_HOME to a profile-scoped path breaks model connections
    # because the profile dir lacks the root .env (API keys), skills/,
    # plugins/, and other infrastructure. The -p flag already activates the
    # profile's config.yaml via _apply_profile_override(). The worker inherits
    # the gateway's HERMES_HOME (root .hermes) which has everything needed.
    if task.tenant:
