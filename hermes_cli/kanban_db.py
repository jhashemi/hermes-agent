    ...
    for row in stale:
        ...
        with write_txn(conn):
            ...
            cur = conn.execute(
                ...
                "claim_expires = NULL, worker_pid = NULL, worker_node = NULL "
                "WHERE id = ? AND status = 'running' AND claim_lock IS ? "
                "AND claim_expires IS NOT NULL AND claim_expires < ?",
...
def promote_task(
    ...
    *,
    ...
    # t_d70258e5 AC1: promote is a status transition — fire the
    # ``kanban_write_op`` seam so the DuckDB mirror sees it.
    _fire_kanban_write_op(
        conn, "promote_task", task_id, result=True, actor=actor,
...
def unblock_task(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        landing_status = "todo" if undone_parents else "ready"
        new_status = (
            "review"
...
def archive_task(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    # t_d70258e5 AC1: archive is a status transition — it MUST fire the
    # ``kanban_write_op`` seam so the DuckDB mirror sees it. (This fire site
    # was missing: the 14-op seam set predated archive_task, so every archive
    # — CLI, dashboard, sweeper — silently skipped the mirror.)
    _fire_kanban_write_op(conn, "archive_task", task_id, result=True)
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    with write_txn(conn):
        rows = conn.execute(
            "SELECT id, worker_pid, claim_lock, started_at, assignee, worker_node "
            "FROM tasks "
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in review_rows:
        ...
        # it carries the review logic (AC verification, merge, etc.).
        #
        # FIX-5 (t_b56c4ca7): peer-review agents do NOT get sdlc-review
        # force-loaded — they inherit whatever the profile ships with.
