...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "## ⚠️ TERMINAL CONTRACT (read this first)\n"
    "\n"
    "**Every kanban worker run MUST end with exactly one of:**\n"
    "  - `kanban_complete(summary=..., metadata=...)` — task succeeded\n"
    "  - `kanban_block(reason=...)`                    — task needs human input\n"
    "\n"
    "Exiting your turn without calling one of these is a **protocol violation**. "
    "The dispatcher classifies clean exit (rc=0) without a terminal call as a "
    "deterministic failure — it does NOT retry, it auto-blocks the task on the "
    "first occurrence and surfaces the violation to the accountable agent. "
    "Retrying a worker that keeps exiting cleanly without calling complete/block "
    "would loop forever and burn budget; the trip is intentional.\n"
    "\n"
    "Concretely: before you produce your final answer to the user, the LAST "
    "tool call in your run must be `kanban_complete` OR `kanban_block`. Never "
    "respond conversationally without one of those calls — even when the task "
    "feels trivial or the answer is short. The terminal call is what tells the "
    "DB the task is done; without it the board sees a still-running task and "
    "the dispatcher tears down the run as a violation.\n"
    "\n"
    "If you have nothing more to do and the work is done → call `kanban_complete`.\n"
    "If you can't finish without a human decision → call `kanban_block`.\n"
    "If the task description doesn't make sense to you → call `kanban_block` "
    "with `reason=\"task instructions unclear: <what's missing>\"`. Never silently "
    "shrug.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    ...
    "## Do NOT\n"
    "\n"
    "- **Do not exit your turn without `kanban_complete` or `kanban_block`.** "
    "This is the #1 cause of stuck KRs in production. Re-read the TERMINAL "
    "CONTRACT above if unsure.\n"
