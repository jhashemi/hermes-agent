...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "Before calling `kanban_complete`, verify ALL of the following. If ANY "
    "step cannot be truthfully answered YES, do NOT call `kanban_complete` — "
    "call `kanban_block(kind='capability', reason='<what failed>')` instead.\n"
    "\n"
    "1. **ARTIFACTS.** Every file you created or modified is listed in "
    "`kanban_complete(artifacts=[<absolute paths>])` or "
    "`metadata.artifacts`. Each path MUST exist on disk at completion.\n"
    "2. **COMMITS.** Every git commit you claim is pushed to origin and "
    "verifiable: `git log origin/<branch> --grep=<hash>` returns non-empty. "
    "List them in `metadata.commit_hashes` as "
    "`[{repo, branch, hash, verified_on_origin}]`.\n"
    "3. **DEPLOYED SERVICES.** For any service you claim is running new code, "
    "record the unit, PID, and start-timestamp in "
    "`metadata.deployed_services` as "
    "`[{host, unit, pid, started_at}]`. Each MUST return `active` from "
    "`systemctl is-active <unit>`.\n"
    "4. **CROSS-HOST PROPAGATION.** If your deliverable must be present on "
    "other hosts (e.g. hermes1 + hermes2), verify the artifact exists on each "
    "target host. Record in `metadata.cross_host_propagation` as "
    "`[{artifact, source_host, target_hosts, mechanism, verified}]`.\n"
    "5. **VERIFICATION EVIDENCE.** Paste the actual command output (git log, "
    "systemctl, curl, stat) into `metadata.verification_evidence` as an "
    "opaque dict of strings. This is your audit trail.\n"
    "6. **TEST PROOF.** If the task specifies test coverage or a test run, "
    "paste the test runner output into `metadata.verification_evidence` so "
    "the green result is on the record.\n"
    "\n"
    "When `kanban.complete_strict_verification` is enabled (config flag, "
    "default OFF during grace period), `kanban_complete` will REFUSE if any "
    "artifacts path does not exist, any commit hash is not found on origin, "
    "any deployed service is not active, or any cross-host propagation is "
    "unverified. When the flag is OFF, a completion that claims work in its "
    "summary but provides no structured metadata gets a WARNING comment "
    "advising the operator of the unverified claim.\n"
    "\n"
    "The class of failure this protocol prevents: a worker marks a task done "
    "claiming it committed/deployed/shipped, but the commit was only on the "
    "worker's local working copy (never pushed), or the artifact exists only "
    "on the worker host (not propagated), or the service was never started. "
    "Every such completion required manual operator substrate-audit to "
    "catch. This protocol makes the primitive catch it."
)
